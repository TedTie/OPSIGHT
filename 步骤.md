...
## 16. 项目链接（PowerShell 示例）
```
cd E:\51\AI\KillerApp
supabase login --no-browser
supabase link --project-ref hmzwgteftyiwfhepkvco
supabase functions list
```

## 17. 设置 Edge Functions Secrets（云端）
```
supabase secrets set \
  SUPABASE_URL="https://hmzwgteftyiwfhepkvco.supabase.co" \
  SERVICE_ROLE_KEY="<你的ServiceRoleKey>" \
  SUPABASE_ANON_KEY="<你的AnonKey>"
supabase secrets list
```
- 说明：`SERVICE_ROLE_KEY` 用于绕过 RLS 执行读写；未设置时函数会退化为 `SUPABASE_ANON_KEY`，部分写操作可能失败。

## 18. 本地启动函数（调试）
```
cd E:\51\AI\KillerApp
$env:SUPABASE_URL = "https://hmzwgteftyiwfhepkvco.supabase.co"
$env:SERVICE_ROLE_KEY = "<你的ServiceRoleKey>"
supabase functions serve killerapp --no-verify-jwt --port 54321
# 调试完成后清理环境变量
Remove-Item Env:SUPABASE_URL
Remove-Item Env:SERVICE_ROLE_KEY
```
- 本地可通过 `http://localhost:54321/killerapp/api/v1/users` 访问调试接口。
...
```
- 生产访问地址（示例）：
  - `https://hmzwgteftyiwfhepkvco.functions.supabase.co/killerapp/api/v1/users`
...
# 设置云端机密（Edge Functions 使用）
supabase secrets set SUPABASE_URL="https://hmzwgteftyiwfhepkvco.supabase.co" SERVICE_ROLE_KEY="<你的ServiceRoleKey>" SUPABASE_ANON_KEY="<你的AnonKey>"

# 本地调试启动（PowerShell 会话注入环境变量）
$env:SUPABASE_URL = "https://hmzwgteftyiwfhepkvco.supabase.co"
$env:SERVICE_ROLE_KEY = "<你的ServiceRoleKey>"
supabase functions serve killerapp --no-verify-jwt --port 54321
